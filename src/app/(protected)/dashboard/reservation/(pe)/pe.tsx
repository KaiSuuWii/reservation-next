...
export default function ReservationPE () {
  ...
  const handleNotificationClicked = async (notification: any) => {
    ...
    const response = await fetch('/api/dashboard/reservation/pe', {
      method: 'PATCH',
      headers: {
  ...
  const handleLetterRequest = async (notification: any) => {
    ...
    try {
      const response = await fetch(
        `/api/dashboard/reservation/pe?id=${notification.reservationNumber}`,
        { method: 'GET' }
      );
  ...
  const getNotifications = async () => {
    ...
    const transformedData = data.map((notification: any) => ({
      ...
      : notification.osa_viewed ? 'update' 
      : notification.pe_acknowledged ? 'reminder' : 'reservation',
      date: notification.date,
      read: notification.pe_viewed,
